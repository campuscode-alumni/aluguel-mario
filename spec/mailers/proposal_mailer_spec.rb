...
describe ProposalMailer, type: :mailer do
  describe 'accept_proposal' do
    it 'render mail successfully' do
      ...
      expect(mail.subject).to eq "Parabéns, sua proposta para o imóvel \
#{proposal.property.title} foi aceita"
    ...
    it 'render email body' do
      ...
      p_1 = "Eu, #{proposal.property.owner.name}, portador(a) do CPF nº #{proposal.property.owner.document}, declaro \
para os devidos
fins que alugo o imóvel sito à #{proposal.property.address}, para #{proposal.user.name},
portador(a) do CPF nº #{proposal.user.document}, entre o período de
#{I18n.localize proposal.start_date} a #{I18n.localize proposal.end_date}, \
pelo valor total de #{number_to_currency proposal.total_amount}."
...
#{proposal.property.usage_rules})
  ...
  describe 'send proposal confirmation' do
    ...
    it 'render email body' do
      ...
      body_2 = %(O usuário #{proposal.user.name} enviou uma proposta com data de inicio em
